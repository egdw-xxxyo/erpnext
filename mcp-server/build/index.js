...
export class ERPNextClient {
	...
	async amendDocument(doctype, name) {
		try {
	...
	async uploadFile(filename, filedata, doctype, docname, isPrivate) {
		try {
	...
	async getChildDocList(doctype, parentDoctype, filters, fields, limit) {
		try {
			const args = {
				doctype,
	...
	return {
		tools: [
				...
				inputSchema: {
					...
					properties: {
						...
						parent_doctype: {
							type: "string",
							description:
								"Parent DocType name — required when querying child table DocTypes (e.g., 'Item Attribute' for 'Item Attribute Value')",
						},
					},
					required: ["doctype"],
				},
				...
				inputSchema: {
					...
					properties: {
						...
						options: {
							type: "string",
							description:
								"Options - for Data with Barcode scanner use 'Barcode', for Link use DocType name, for Select use newline-separated values",
						},
						insert_after: {
							type: "string",
			...
			},
			{
				name: "call_method",
				description:
					"Call a Frappe whitelisted method (API endpoint). Useful for: frappe.client.get (fetch full doc), frappe.client.insert (create with child tables), frappe.client.get_count, frappe.client.get_list. Args are passed as POST body. For frappe.client.insert, pass {doc: {doctype: 'X', ...fields, child_table: [{...}]}}.",
				inputSchema: {
					type: "object",
					properties: {
						method: {
							type: "string",
							description:
								"Full method path (e.g., frappe.client.get_count, erpnext.manufacturing.page.workplace_portal.workplace_portal.get_workplaces)",
						},
						args: {
							type: "object",
							additionalProperties: true,
			...
			},
			{
				...
				description:
					"Submit a draft document in ERPNext (sets docstatus=1). The document must be in Draft state (docstatus=0). This is equivalent to clicking the Submit button in the UI. Works for Stock Entry, Sales Invoice, Purchase Invoice, Journal Entry, Work Order, BOM, etc.",
				inputSchema: {
					type: "object",
					properties: {
						doctype: {
			...
			},
			{
				...
				description:
					"Cancel a submitted document in ERPNext (sets docstatus=2). The document must be in Submitted state (docstatus=1). This is equivalent to clicking Amend > Cancel in the UI.",
				inputSchema: {
					type: "object",
					properties: {
						doctype: {
			...
			},
			{
				name: "amend_document",
				description:
					"Amend a cancelled document in ERPNext. Creates a new copy of the cancelled document in Draft state that can be modified and resubmitted.",
				inputSchema: {
					type: "object",
					properties: {
						doctype: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	switch (request.params.name) {
		...
		case "add_custom_field": {
			...
			const insert_after = request.params.arguments?.insert_after;
			const reqd = request.params.arguments?.reqd;
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `Failed to add custom field to ${doctype}: ${
		...
		case "get_doctype_fields": {
			...
			try {
				...
				const fields = (meta.docs?.[0]?.fields || []).map((f) => ({
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `Failed to call ${method}: ${error?.message || "Unknown error"}`,
						},
					],
					isError: true,
				};
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `Amended ${doctype}: ${name}\n\nNew draft: ${JSON.stringify(
								result,
								null,
								2
							)}`,
				...
				return {
					content: [
						{
							type: "text",
							text: `Failed to amend ${doctype} ${name}: ${error?.message || "Unknown error"}`,
						},
					],
					isError: true,
				};
		...
		case "upload_file": {
			...
			const docname = request.params.arguments?.docname;
			const is_private = request.params.arguments?.is_private;
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `Failed to upload ${filename}: ${error?.message || "Unknown error"}`,
						},
					],
					isError: true,
				};
