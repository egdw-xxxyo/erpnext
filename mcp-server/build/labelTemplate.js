...
export const LABEL_TEMPLATE_TOOLS = [
    {
        ...
        description: "List Label Templates with name, label_size, template_type. Use before editing to find the right template.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "get_label_template",
        description: "Read a Label Template — html_template, padding fields, field_mapping, preview_data, label_size. Use this before editing to see the current state.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                html_template: {
                    type: "string",
                    description: "Body HTML only. Children with position:absolute;width:100%;height:100% fill the padded area, not the full label. Use <table> for layout (wkhtmltoimage has no flexbox).",
                },
    ...
    },
    {
        ...
        description: "Create or update a Label Template Example. Use this to grow the shared snippet library — examples surface in the in-app reference dialog and to subsequent get_label_template_reference calls. If a record with the given title exists, it is updated; otherwise a new one is created.",
        inputSchema: {
            type: "object",
            properties: {
                title: { type: "string" },
...
export function isLabelTemplateTool(name) {
...
export async function handleLabelTemplateTool(name, args, erpnext) {
    ...
    try {
        switch (name) {
            case "list_label_templates": {
                const docs = await erpnext.getDocList("Label Template", undefined, ["name", "label_size", "template_type", "reference_doctype"], 200);
                return json(docs);
            }
            ...
            case "get_label_template_reference": {
                const result = await erpnext.callMethod("erpnext.manufacturing.doctype.label_template.label_template.get_template_reference");
            ...
            case "list_label_template_examples": {
                ...
                const includeUnpublished = arg(args, "include_unpublished") ?? false;
                const filters = {};
                if (!includeUnpublished)
                    filters.is_published = 1;
                if (category)
                    filters.category = category;
                const docs = await erpnext.getDocList("Label Template Example", filters, ["title", "category", "description_uk", "html_snippet", "notes", "display_order", "is_published"], 500);
                return json(docs);
            }
            case "upsert_label_template_example": {
                const title = arg(args, "title");
                const category = arg(args, "category");
                const html_snippet = arg(args, "html_snippet");
                ...
                const data = {
                    ...
                    category,
                    html_snippet,
                    description_uk: arg(args, "description_uk"),
                    notes: arg(args, "notes"),
                    display_order: arg(args, "display_order") ?? 0,
                    is_published: arg(args, "is_published") === false ? 0 : 1,
                };
                let exists = true;
                try {
