...
`.trim();
const VERSION_NOTE = "Optional. Target version (e.g. 'v2'). Defaults to the doc's default version (the one runtime uses).";
export const SCRIPT_TOOLS = [
    ...
    },
    {
        ...
        description: "Read a Workplace Script — main `script`, all state rows (with their per-state scripts), and transitions. By default returns the snapshot of the version that runtime uses (the default version). Pass `version` to read a specific snapshot. Always returns version metadata: default_version, viewing_version, versions[].",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Create a new version on a Workplace Script. Auto-named v1, v2, … Snapshot is copied from `source_version` (defaults to the current default version). The new version is NOT made default — call set_default_workplace_script_version afterwards if you want runtime to use it.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: `Create or update a State row on a Workplace Script. If the state already exists, updates its script (and flags if provided). If not, appends a new row. Targets the default version unless \`version\` is supplied.\n\n${STATE_SCRIPT_USAGE}`,
        inputSchema: {
            type: "object",
            properties: {
    ...
    // ── Scanner Script ──────────────────────────────────────────────────────
    {
        ...
        description: "List all Scanner Scripts (reusable Python modules loaded into `scripts.<name>` inside every Workplace Script execution).",
        inputSchema: { type: "object", properties: {} },
    },
    {
    ...
    },
    {
        ...
        description: "Create a new version on a Scanner Script. Auto-named v1, v2, … Snapshot is copied from `source_version` (defaults to current default version). Not made default automatically.",
        inputSchema: {
            type: "object",
            properties: {
                name: { type: "string" },
    ...
    },
    {
        ...
        description: "Create or update a Scanner Script. The script is a Python module — define top-level functions / constants. Inside Workplace Script state code, access symbols via `scripts.<name>.<symbol>` (the name is lowercased with spaces/dashes → underscores). On existing docs, targets the default version unless `version` is supplied.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    // ── Reflectometer Script ────────────────────────────────────────────────
    {
        ...
        description: "List all Reflectometer Scripts (run automatically after each OTDR measurement upload). Same versioning model as Scanner Script. Entry point: `on_event(ctx)` or `on_reflectometer(ctx)`; ctx exposes ctx.otdr, ctx.log_entry, ctx.payload (parsed SOR dict).",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "get_reflectometer_script",
        description: "Read a Reflectometer Script — name, is_active, full script body. Defaults to the default version's snapshot. Pass `version` for a specific snapshot.",
        inputSchema: {
            type: "object",
            properties: {
                name: { type: "string" },
    ...
    },
    {
        ...
        description: "Create or update a Reflectometer Script. Define `def on_event(ctx)` (or `on_reflectometer(ctx)`). ctx fields: ctx.otdr (OTDR doc), ctx.log_entry (just-saved measurement row), ctx.payload (parsed SOR dict). Frappe and json modules in scope. Errors are logged to Error Log, never raised back to the desktop client.",
        inputSchema: {
            type: "object",
            properties: {
                name: { type: "string" },
    ...
    },
    {
        ...
        description: "Create or update a Scanner Command. State scripts match it via `e.scan_type == \"command\" and e.doc.barcode_id == \"CMD-...\"`. Stack actions (Push/Pop/Toggle) live on the root Workplace Script's Subflow Entries table, not here.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Replace the entire Subflow Entries table on a (root) Workplace Script. Pass all rows. Each row: from_state (required, state name in this script), trigger_type ('Command' | 'Scan Type'), trigger_value (barcode_id or scan_type), target_subflow (required). On match, scanner switches to target subflow's initial state and processes the same scan there. Subflow exits via e.state.clear().",
        inputSchema: {
            type: "object",
            properties: {
                script_name: { type: "string" },
    ...
    // ── Workplace Script meta (top-level fields) ─────────────────────────────
    {
        ...
        description: "Update top-level (non-versioned) fields on a Workplace Script: workplace, is_active, parent_script. parent_script makes this script a subflow of another Workplace Script — subflows have no workplace and are exempt from the per-workplace uniqueness check. Use create_document via the generic ERPNext MCP to create a new Workplace Script.",
        inputSchema: {
            type: "object",
            properties: {
                script_name: { type: "string" },
...
function findVersion(doc, version) {
    const versions = doc.versions || [];
    if (version)
        return versions.find((v) => v.version === version);
    return versions.find((v) => v.is_default) || versions[0];
}
function workplaceSnapshot(row) {
...
export async function handleScriptTool(name, args, erpnext) {
    ...
    try {
        switch (name) {
            ...
            case "list_workplace_scripts": {
                const docs = await erpnext.getDocList("Workplace Script", undefined, ["name", "workplace", "parent_script", "is_active", "default_version"], 200);
                return json(docs);
            }
            case "get_workplace_script": {
                const scriptName = arg(args, "script_name");
                const version = arg(args, "version");
                if (!scriptName)
                    return err("script_name is required");
                const doc = await erpnext.getDocument("Workplace Script", scriptName);
                const row = findVersion(doc, version);
                if (!row)
                    return err(`Version ${version || "default"} not found on ${scriptName}`);
            ...
            case "add_workplace_script_version": {
                const scriptName = arg(args, "script_name");
                const sourceVersion = arg(args, "source_version");
                const label = arg(args, "label");
                if (!scriptName)
                    return err("script_name is required");
                const doc = await erpnext.getDocument("Workplace Script", scriptName);
                const versions = (doc.versions || []).map((v) => ({ ...v }));
                const sourceRow = findVersion({ versions }, sourceVersion);
                if (!sourceRow)
                    return err(`Source version ${sourceVersion || "default"} not found`);
            ...
            case "remove_workplace_script_version": {
                ...
                if (!scriptName || !version)
                    return err("script_name and version are required");
                const doc = await erpnext.getDocument("Workplace Script", scriptName);
                const versions = (doc.versions || []).map((v) => ({ ...v }));
                if (versions.length <= 1)
                    return err("Cannot remove the only version");
                const row = versions.find((v) => v.version === version);
                if (!row)
                    return err(`Version ${version} not found`);
                if (row.is_default) {
                ...
                const payload = { versions: filtered };
                if (doc.viewing_version === version)
                    payload.viewing_version = doc.default_version;
            ...
            case "set_default_workplace_script_version": {
                ...
                if (!versions.find((v) => v.version === version))
                    return err(`Version ${version} not found`);
                for (const v of versions)
                    v.is_default = v.version === version ? 1 : 0;
            ...
            case "edit_workplace_script_main": {
                const scriptName = arg(args, "script_name");
                const script = arg(args, "script");
                const version = arg(args, "version");
                if (!scriptName || script === undefined)
                    return err("script_name and script are required");
                const doc = await erpnext.getDocument("Workplace Script", scriptName);
                const row = findVersion(doc, version);
                if (!row)
                    return err(`Version ${version || "default"} not found`);
                const snap = workplaceSnapshot(row);
                await erpnext.updateDocument("Workplace Script", scriptName, {
            ...
            case "edit_workplace_script_state": {
                const scriptName = arg(args, "script_name");
                const stateName = arg(args, "state");
                const version = arg(args, "version");
                if (!scriptName || !stateName)
                    return err("script_name and state are required");
                const doc = await erpnext.getDocument("Workplace Script", scriptName);
                const row = findVersion(doc, version);
                if (!row)
                    return err(`Version ${version || "default"} not found`);
                const snap = workplaceSnapshot(row);
                const states = (snap.states || []).map((s) => ({ ...s }));
                const stateScript = arg(args, "script");
                const label = arg(args, "label");
                const isInitial = arg(args, "is_initial");
                const isFinal = arg(args, "is_final");
                const idx = states.findIndex((s) => s.state === stateName);
                let action;
                if (idx >= 0) {
                    if (stateScript !== undefined)
                        states[idx].on_enter_script = stateScript;
                    if (label !== undefined)
                        states[idx].label = label;
                    if (isInitial !== undefined)
                        states[idx].is_initial = isInitial ? 1 : 0;
                    if (isFinal !== undefined)
                        states[idx].is_final = isFinal ? 1 : 0;
                    action = "updated";
                }
                else {
                    states.push({
            ...
            case "delete_workplace_script_state": {
                ...
                if (!row)
                    return err(`Version ${version || "default"} not found`);
                const snap = workplaceSnapshot(row);
                const states = (snap.states || []).filter((s) => s.state !== stateName);
            ...
            case "edit_workplace_script_transitions": {
                const scriptName = arg(args, "script_name");
                const transitions = arg(args, "transitions");
                const version = arg(args, "version");
            ...
            case "list_scanner_scripts": {
                const docs = await erpnext.getDocList("Device Script", { script_type: "Scanner" }, ["name", "is_active", "default_version", "script_type"], 200);
                return json(docs);
            }
            case "list_reflectometer_scripts": {
                const docs = await erpnext.getDocList("Device Script", { script_type: "Reflectometer" }, ["name", "is_active", "default_version", "script_type"], 200);
                return json(docs);
            }
            case "get_reflectometer_script": {
                ...
                if (doc.script_type !== "Reflectometer")
                    return err(`${n} is not a Reflectometer script`);
                const row = findVersion(doc, version);
                if (!row)
                    return err(`Version ${version || "default"} not found on ${n}`);
            ...
            case "edit_reflectometer_script": {
                const n = arg(args, "name");
                const script = arg(args, "script");
                const isActive = arg(args, "is_active");
                const version = arg(args, "version");
                if (!n || script === undefined)
                    return err("name and script are required");
                let exists = true;
                let doc = null;
                try {
                ...
                if (exists) {
                    if (doc.script_type !== "Reflectometer")
                        return err(`${n} exists but is not a Reflectometer script`);
                    const row = findVersion(doc, version);
                    if (!row)
                        return err(`Version ${version || "default"} not found on ${n}`);
                    const data = {
            ...
            case "remove_scanner_script_version": {
                ...
                const version = arg(args, "version");
                if (!n || !version)
                    return err("name and version are required");
                const doc = await erpnext.getDocument("Device Script", n);
                const versions = (doc.versions || []).map((v) => ({ ...v }));
                if (versions.length <= 1)
                    return err("Cannot remove the only version");
            ...
            case "list_scanner_commands": {
                const docs = await erpnext.getDocList("Scanner Command", undefined, ["name", "barcode_id", "description"], 200);
                return json(docs);
            }
            case "edit_scanner_command": {
                const n = arg(args, "name");
                const barcodeId = arg(args, "barcode_id");
                ...
                if (exists) {
                    ...
                    if (Object.keys(data).length === 0)
                        return err("Nothing to update — provide barcode_id or description");
            ...
            case "list_subflow_entries": {
                ...
                return json({
                    name: doc.name,
                    entries: (doc.subflow_entries || []).map((r) => ({
            ...
            case "set_subflow_entries": {
                const scriptName = arg(args, "script_name");
                const entries = arg(args, "entries");
                if (!scriptName)
                    return err("script_name is required");
                if (!Array.isArray(entries))
                    return err("entries must be an array");
                const rows = entries.map((r) => {
                    if (!r.from_state)
                        throw new Error("from_state is required");
                    if (!r.target_subflow)
                        throw new Error("target_subflow is required");
                    return {
                        from_state: r.from_state,
                        trigger_type: r.trigger_type,
                        trigger_value: r.trigger_value,
            ...
            case "edit_workplace_script_meta": {
                const scriptName = arg(args, "script_name");
                const workplace = arg(args, "workplace");
                const isActive = arg(args, "is_active");
                const parentScript = arg(args, "parent_script");
                if (!scriptName)
                    return err("script_name is required");
                const data = {};
                if (workplace !== undefined)
                    data.workplace = workplace || null;
                if (isActive !== undefined)
                    data.is_active = isActive ? 1 : 0;
                if (parentScript !== undefined)
                    data.parent_script = parentScript || null;
                if (Object.keys(data).length === 0)
                    return err("Nothing to update — provide workplace, is_active, or parent_script");
                if (data.parent_script && (data.workplace || (data.workplace === undefined))) {
                    // subflow must have no workplace
                    if (data.workplace)
                        return err("Subflow (parent_script set) cannot have a workplace");
                }
