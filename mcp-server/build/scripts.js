...
export const SCRIPT_TOOLS = [
	// ── Workplace Script ────────────────────────────────────────────────────
	{
		...
		description:
			"List all Workplace Scripts (the per-workplace scanner pipeline scripts) with workplace + parent_script + active status + default version. parent_script identifies subflows (see edit_workplace_script_meta).",
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				...
				source_version: {
					type: "string",
					description: "Version to clone the snapshot from. Defaults to default version.",
				},
	...
	},
	{
		...
		description:
			"Remove a version row from a Workplace Script. Blocked when only one version exists or when removing the current default (switch default first).",
		inputSchema: {
			type: "object",
			properties: {
				script_name: { type: "string" },
	...
	},
	{
		...
		description:
			"Mark a version as the default (the one runtime executes). The previous default becomes a draft.",
		inputSchema: {
			type: "object",
			properties: {
				script_name: { type: "string" },
		...
		inputSchema: {
			...
			properties: {
				...
				script: {
					type: "string",
					description:
						"Python source defining `def on_scan(e): ...`. Stored on the row's on_enter_script field.",
				},
				label: {
					type: "string",
					description: "Optional translatable label shown in the diagram (defaults to state name)",
				},
				is_initial: {
					type: "boolean",
					description: "Mark as the initial state. Only one state per script may be initial.",
				},
	...
	},
	{
		...
		description:
			"Remove a State row from a Workplace Script. Targets the default version unless `version` is supplied.",
		inputSchema: {
			type: "object",
			properties: {
				script_name: { type: "string" },
					...
					items: {
						...
						properties: {
							...
							event: {
								type: "string",
								description:
									"Human-readable label (e.g. start_packaging) shown on the diagram edge.",
							},
	...
	},
	{
		...
		description:
			"Read a Scanner Script — name, is_active, full script body. By default returns the snapshot of the version that runtime uses (default). Pass `version` to read a specific snapshot. Always returns default_version, viewing_version, versions[].",
		inputSchema: {
			type: "object",
			properties: {
				name: { type: "string" },
	...
	},
	{
		...
		description:
			"Remove a version row from a Scanner Script. Blocked when only one version exists or when removing the current default.",
		inputSchema: {
			type: "object",
			properties: {
				name: { type: "string" },
				...
				entries: {
					...
					items: {
						...
						properties: {
							from_state: {
								type: "string",
								description: "State in this script that triggers entry.",
							},
		...
		inputSchema: {
			...
			properties: {
				...
				workplace: {
					type: "string",
					description:
						"Workplace link, or null/empty to clear. Mutually exclusive with parent_script.",
				},
				is_active: { type: "boolean" },
				parent_script: {
					type: "string",
					description:
						"Workplace Script name. Setting marks this as a subflow. Pass empty string to clear.",
				},
			},
			required: ["script_name"],
		},
	...
	try {
		switch (name) {
			...
			case "add_workplace_script_version": {
				...
				return ok(
					`Added version ${newName} on Workplace Script ${scriptName} (cloned from ${sourceRow.version})`
				);
			}
			case "remove_workplace_script_version": {
				...
				if (row.is_default) {
					return err(
						`Cannot remove the default version ${version}. Switch default first with set_default_workplace_script_version.`
					);
				}
			...
			case "set_default_workplace_script_version": {
				...
				const doc = await erpnext.getDocument("Workplace Script", scriptName);
				const versions = (doc.versions || []).map((v) => ({ ...v }));
				if (!versions.find((v) => v.version === version)) return err(`Version ${version} not found`);
			...
			case "edit_workplace_script_transitions": {
				...
				return ok(
					`Replaced ${transitions.length} transition(s) on ${scriptName} (version ${row.version})`
				);
			}
			// ── Scanner Script ──────────────────────────────────────────────────
			...
			case "add_scanner_script_version": {
				...
				return ok(
					`Added version ${newName} on Scanner Script ${n} (cloned from ${sourceRow.version})`
				);
			}
