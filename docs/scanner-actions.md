...
11. [Конфігурація фізичного сканера (Hardware Scanner Configuration)](#11-конфігурація-фізичного-сканера-hardware-scanner-configuration)

---

## 1. Огляд (Overview)

Система дій сканера дозволяє апаратним сканерам штрих-кодів взаємодіяти з ERPNext через HTTP-ендпоінт. Кожен сканер автентифікується за допомогою HMAC-ключа (виведеного з API Key сканера та спільного секрету) та виконує налаштовані дії: запуск/завершення Карток завдань (Job Cards), пошук за серійними номерами (Serial Numbers), сканування матеріалів тощо.
...
| Компонент | Опис |
|-----------|------|
| Налаштування сканера (Scanner Setup) | Конфігурація сканера: HMAC-ключ (обчислюється), користувачі, дії |
...
|----------|-------------|------|
| `scanner_key` | Так | HMAC-ключ сканера (16 символів hex) |
| `data` | Так | Дані сканування (штрих-код) |
| `action` | Ні | Явне визначення дії (обходить автовизначення) |

### Автентифікація (Authentication)

Двокомпонентна автентифікація: **API Key** (публічний, на формі сканера) + **Secret** (приватний, в `site_config.json` і в прошивці сканера).

1. Метод позначений `@frappe.whitelist(allow_guest=True)` — сесія Frappe не потрібна
2. Сканер обчислює `scanner_key = HMAC-SHA256(secret, api_key)[:16]` і надсилає його
3. Сервер перебирає всі активні сканери, обчислює `HMAC-SHA256(scanner_secret, api_key)` для кожного, порівнює через `hmac.compare_digest` (захист від timing-атак)
4. Якщо збіг не знайдено → HTTP 403
5. Після автентифікації: `frappe.set_user()` встановлюється на працівника сканера

#### Компоненти автентифікації

| Компонент | Де зберігається | Видимість | Опис |
|-----------|----------------|-----------|------|
| `api_key` | БД (`tabScanner Setup.api_key`) | Публічний — на формі + QR-код | 8 символів hex, ідентифікує сканер |
| `scanner_secret` | `site_config.json` + прошивка сканера | Приватний | Єдиний секрет для всіх сканерів |
| `scanner_key` | Не зберігається — обчислюється | Передається в запиті | `HMAC-SHA256(secret, api_key)[:16]` |

#### Як працює автентифікація

```
1. Адміністратор створює Scanner Setup → генерується api_key (напр. "a1b2c3d4")
2. Адміністратор сканує QR-код з api_key у фізичний сканер
3. Фізичний сканер вже має scanner_secret у своїй конфігурації
4. При скануванні штрих-коду, сканер обчислює:
   scanner_key = HMAC-SHA256(scanner_secret, "a1b2c3d4")[:16] → "f9e8d7c6b5a43210"
5. Сканер надсилає: handle_scan(scanner_key="f9e8d7c6b5a43210", data="JC-MFG-00042")
6. Сервер перебирає активні сканери, знаходить api_key="a1b2c3d4", перевіряє HMAC → збіг
```

#### Налаштування секрету (Setting Up the Secret)

Додати в `site-config.json` (розгортається через `./deploy`):

```json
{
  "scanner_secret": "e5779f530bf31dc6724e96afa6448cd1889e996e8fc3efb1bf36b0a74a3be461"
}
```

Цей самий секрет потрібно налаштувати в прошивці/конфігурації кожного фізичного сканера.

Зміна `scanner_secret` інвалідує **всі** сканери одночасно — потрібно оновити секрет на кожному пристрої.

#### QR-код та відображення ключа

На формі Scanner Setup відображається `api_key` (8 символів) та його QR-код. API key — публічний ідентифікатор, без секрету він не дає доступу.
...
## 11. Конфігурація фізичного сканера (Hardware Scanner Configuration)

Фізичний сканер зберігає в пам'яті три параметри:
...
- **URL** та **API Key** — встановлюються через сканування командних штрих-кодів. Можна швидко змінити: переключити сканер з prod на test, оновити ключ після регенерації.
- **Секрет** — прихований, прошивається один раз. Однаковий для всіх сканерів на одному сайті. Зміна секрету вимагає перепрошивки всіх пристроїв.

### Командні штрих-коди для конфігурації

Для зміни URL або API Key без перепрошивки використовуються спеціальні командні штрих-коди:

| Штрих-код | Дія | Наступне сканування |
|-----------|-----|---------------------|
| `CFG-URL` | Перейти в режим зміни URL | QR-код з новим URL сервера |
| `CFG-KEY` | Перейти в режим зміни API Key | QR-код з API Key (з форми Scanner Setup) |

**Приклад: переключення сканера з prod на test**

```
1. Сканувати: CFG-URL
2. Сканувати: QR-код "https://test.example.com/api/method/..."
3. Сканувати: CFG-KEY
4. Сканувати: QR-код з тестового Scanner Setup (api_key нового середовища)
→ Сканер тепер працює з тестовим сервером
```

**Приклад: оновлення API Key після регенерації**

```
1. Адміністратор натискає "Regenerate API Key" на формі Scanner Setup
2. Сканувати: CFG-KEY
3. Сканувати: QR-код з оновленим api_key
→ Сканер використовує новий ключ
```

### Алгоритм запиту сканера

При кожному скануванні штрих-коду сканер виконує:

```
1. Обчислити: scanner_key = HMAC-SHA256(secret, api_key)[:16]
2. Надіслати HTTP GET/POST:
   {url}?scanner_key={scanner_key}&data={відсканований_штрих-код}
3. Обробити JSON-відповідь (показати prompt/message на екрані)
```
