frappe.ui.form.on("Label Printer", {
	refresh(frm) {
		...
		frm.add_custom_button(
			__("Clear Memory"),
			() => {
				frappe.confirm(__("Erase all stored images from printer memory?"), () => {
					frappe.call({
						...
						callback(r) {
							if (r.message && r.message.success) {
								frappe.show_alert({
									message: __("Printer memory cleared!"),
									indicator: "green",
								});
							}
						},
							...
							callback(r) {
								if (r.message) {
									frappe.msgprint({
										...
										message: `<b>${__("Command")}:</b> ${r.message.command}<br><pre>${
											r.message.response || "(no response)"
										}</pre>`,
									});
								}
							},
	...
	$wrapper.find(".btn-check-conn").on("click", function () {
		...
		frappe.call({
			...
			error(r) {
				...
				$btn.prop("disabled", false).text(__("Check Connection"));
				$result.html(
					`<span class="indicator-pill red">${__("Connection failed")} (${elapsed}s)</span>`
				);
			},
		});
	});
