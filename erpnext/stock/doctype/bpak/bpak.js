...
function render_packed_summary(frm) {
	let wrapper = frm.fields_dict.packed_summary_html && frm.fields_dict.packed_summary_html.$wrapper;
	if (!wrapper) return;
	if (frm.is_new()) {
		wrapper.empty();
		return;
		...
		callback: function (r) {
			...
			if (!d.packages.length) {
				...
				d.packages.forEach((pkg) => {
					let items = pkg.items.length
						? pkg.items
						: [{ item_code: "", item_name: "", qty: 0, serial_no: "" }];
					items.forEach((it, idx) => {
						html += "<tr>";
						if (idx === 0) {
							let pkg_link = `<a href="/app/package/${encodeURIComponent(
								pkg.name
							)}">${frappe.utils.escape_html(pkg.name)}</a>`;
							html += `<td rowspan="${items.length}">${pkg_link}</td>`;
							html += `<td rowspan="${items.length}">${frappe.utils.escape_html(
								pkg.status || ""
							)}</td>`;
						}
						let item_label = it.item_code
							? `${frappe.utils.escape_html(it.item_code)}${
									it.item_name ? " — " + frappe.utils.escape_html(it.item_name) : ""
							  }`
							: `<span class="text-muted">${__("(empty)")}</span>`;
						html += `<td>${item_label}</td>`;
						let right = it.serial_no ? frappe.utils.escape_html(it.serial_no) : it.qty;
			...
			let pct = d.planned_total ? Math.round((d.packed_total / d.planned_total) * 100) : 0;
