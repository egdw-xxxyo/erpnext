...
frappe.query_reports["Monthly Attendance Sheet"] = {
	...
	onload: function (report) {
		...
		return frappe.call({
			method: "erpnext.payroll_ua.report.monthly_attendance_sheet.monthly_attendance_sheet.get_attendance_years",
			callback: function (r) {
...
function grab_scrollbar(event, strip, body) {
	...
	const grip =
		event.target === thumb ? event.clientX - thumb.getBoundingClientRect().left : thumb.offsetWidth / 2;
...
function get_mark_html(value, mark) {
	const note = mark.note
		? `<span class="hours ${mark.note.kind}">${frappe.utils.escape_html(mark.note.text)}</span>`
		: "";
