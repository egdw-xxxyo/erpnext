...
const METHOD = "erpnext.payroll_ua.page.attendance_sheet.attendance_sheet";

const EXPORT_METHOD = "erpnext.payroll_ua.attendance_export.download_sheet";
...
class AttendanceSheet {
	...
	make_body() {
		this.$container = $('<div class="attendance-sheet-container"></div>').appendTo(this.page.main);
	...
	render() {
		...
		this.$table.html(this.summarized.get_value() ? this.get_summary_html() : this.get_sheet_html());
		this.size_table();
	}
	...
	grab_scrollbar(event) {
		...
		const drag = (moved) => {
			...
			table.scrollLeft = span ? (offset / span) * (table.scrollWidth - table.clientWidth) : 0;
			this.paint_scrollbar();
		};
	...
	open_selection_menu(event, employees, dates) {
		...
		const cell =
			employees.length === 1 && dates.length === 1 ? this.get_cell(employees[0], dates[0]) : null;
		...
		return show_menu(event, items.concat(this.get_document_items(cell)), () => this.clear_selection());
	}
	...
	get_document_items(cell) {
		...
		if (cell.leave_application)
			items.push({
				label: __("Open Leave"),
				onclick: () => frappe.set_route("Form", "Leave Application", cell.leave_application),
			});
	...
	clear_range(employees, period) {
		const span =
			period.from_date === period.to_date
				...
				: `${frappe.datetime.str_to_user(period.from_date)} — ${frappe.datetime.str_to_user(
						period.to_date
				  )}`;

		const message =
			employees.length === 1
				? __("Delete everything marked for {0} on {1}?", [this.employee_name(employees[0]), span])
				: __("Delete everything marked for {0} employees on {1}?", [employees.length, span]);
...
function get_hours_html(cell) {
	if (cell.overtime_hours) return `<span class="hours over">+${format_hours(cell.overtime_hours)}</span>`;
...
function get_cell_title(cell) {
	...
	return [cell.status ? __(cell.status) : "", hours, cell.shift, cell.locked ? __("Approved") : ""]
		.filter(Boolean)
		.join(" · ");
}
...
function get_totals(row) {
	...
	return Object.fromEntries(Object.entries(totals).map(([key, value]) => [key, format_hours(value)]));
}

function month_start() {
	const today = frappe.datetime.str_to_obj(frappe.datetime.get_today());

	return frappe.datetime.obj_to_str(new Date(today.getFullYear(), today.getMonth(), 1)).slice(0, 10);
}
...
function report_removal({ deleted, skipped }) {
	...
	if (skipped.length) report_skipped(skipped, deleted ? "orange" : "red", __("Days that were kept"));
}
