...
def create_v16_ported_fields():
	...
	On version-16 the stock JSONs are kept pristine (see CLAUDE.md: never edit a stock
	DocType JSON), so these ship as Custom Fields instead."""
	fields = [
		{
			"dt": "Supplier",
			"description": "Short code used in serial number generation (e.g., 'K' for a supplier)",
			"fieldname": "abbr",
			"fieldtype": "Data",
			"label": "Abbreviation",
			"length": 10,
		...
		{
			"dt": "Lead",
			"description": "Fill in when the organization is not a customer yet. A Lead may link either a Prospect or a Customer, not both.",
			"fieldname": "prospect",
			"fieldtype": "Link",
			"label": "Prospect",
			"options": "Prospect",
		...
		},
		{
			...
			"insert_after": "conversion_probability",
		},
		{
			"dt": "Lead",
		...
		{
			"dt": "Lead",
			"depends_on": "eval:['Not Relevant', 'Lost'].includes(doc.status) || doc.close_reason",
			"fieldname": "close_section",
			"fieldtype": "Section Break",
		...
		},
		{
			...
			"fieldtype": "Small Text",
			"label": "Close Reason",
			"insert_after": "close_section",
		},
		{
			"dt": "Lead",
			"collapsible": 1,
			"depends_on": "eval:(doc.status_reversals || []).length",
			"fieldname": "reversal_section",
			"fieldtype": "Section Break",
			"label": "Status Reversal History",
			"insert_after": "close_reason",
		},
		{
			"dt": "Lead",
			"fieldname": "status_reversals",
			"fieldtype": "Table",
			"label": "Status Reversals",
			"no_copy": 1,
			"options": "Lead Status Reversal",
			"read_only": 1,
			"insert_after": "reversal_section",
		},
		{
			"dt": "Job Card",
			"fieldname": "auto_stock_entry",
			"fieldtype": "Link",
			"label": "Auto Stock Entry",
			"options": "Stock Entry",
			"read_only": 1,
			"hidden": 1,
			"no_copy": 1,
			"description": "Stock Entry auto-created on Job Card submit when Operation has Finishes to Stock enabled.",
			"insert_after": "serial_no",
		},
		{
			"dt": "Item",
			"depends_on": "has_serial_no",
			"fieldname": "serial_number_template",
			"fieldtype": "Link",
			"label": "Serial Number Template",
			"options": "Serial Number Template",
			"insert_after": "has_serial_no",
		},
		{
			"dt": "Item",
			"depends_on": "has_variants",
			"description": "Pattern for variant item code/name. Use {AttributeName} placeholders. Values are resolved from Short Name (falls back to Abbreviation). Example: БпЛА Укропчик {Розмір рами} {Номер ТУ} {Тип камери} {Призначення}",
			"fieldname": "variant_name_pattern",
			"fieldtype": "Data",
			"label": "Variant Name Pattern",
			"insert_after": "variant_based_on",
		},
		{
			"dt": "Item Attribute Value",
			"description": "Short display name used in technical code patterns (Specification Number Template). Falls back to abbreviation if empty.",
		...
		{
			"dt": "Item Attribute Value",
			"description": "Human-readable label used in variant name patterns (e.g. 'прот', 'терм'). Falls back to short_name, then abbreviation.",
			"fieldname": "display_name",
			"fieldtype": "Data",
			"in_list_view": 1,
			"label": "Display Name",
			"insert_after": "short_name",
		},
		{
			"dt": "Item Attribute Value",
			"description": "Optional. When this attribute value is used in a variant, the linked item will replace the template item in the BOM.",
...
def set_v16_ported_properties():
	"""Property Setters replacing stock-JSON property tweaks carried on the v15 fork."""
	props = [
		("Item", "serial_no_series", "read_only_depends_on", "serial_number_template", "Data"),
	...
	# Quotation keeps a change log on the v15 fork.
	if not frappe.db.exists("Property Setter", "Quotation-main-track_changes"):
		frappe.get_doc(
			{
				"doctype": "Property Setter",
